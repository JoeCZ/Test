...
public class Question {
	...
	static TreeNode createBST(ArrayList<Integer> list) {
		if(list.isEmpty()) return null;
		int size = list.size();
		TreeNode root = new TreeNode(list.get(size/2));
		...
		root.left = createBST(lList);
		root.right = createBST(rList);
		return root;
	}
}
